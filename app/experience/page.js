    ...
    return (
        <div className="min-h-screen py-10 bg-white dark:bg-gray-900">
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                <SectionHeader eyebrow="Career" title="Professional Experience" description="My journey as a Cloud Data Engineer" />
                            ...
                            <div key={index} className="mb-8 ml-6">
                                ...
                                <div className="p-3 bg-white rounded-lg border border-gray-200 shadow-sm dark:bg-gray-800 dark:border-gray-700">
                                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-2">
                                        <h3 className="text-sm font-semibold text-gray-900 dark:text-white">{exp.title} at {exp.company}</h3>
                                        <span className="bg-blue-50 text-blue-700 text-xs font-medium mt-1 sm:mt-0 px-2 py-0.5 rounded-md dark:bg-blue-900 dark:text-blue-300">{exp.period}</span>
                                    </div>
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">{exp.location}· {exp.duration}</p>
                                    <ul className="space-y-1.5 mb-3">
                                        {exp.description.map((item, i) => (
                                            <li key={i} className="text-xs text-gray-600 dark:text-gray-300 pl-3 relative before:content-['•'] before:absolute before:left-0 before:text-gray-400">{item}</li>
                                        ))}
                                    </ul>
                                    <div className="flex flex-wrap gap-1.5 mt-2">
                                        {exp.technologies.map((tech, i) => (
                                            <span key={i} className="px-1.5 py-0.5 text-xs font-medium bg-gray-50 text-gray-600 rounded-md dark:bg-gray-700 dark:text-gray-300">{tech}</span>
                                        ))}
                                    </div>
                                </div>
                            </div>
